...
// ================= TELEGRAM COMMAND + AUTO BALANCE ADD-ON =================

// ===== USER STATE =====
let userThresholdUSDT = null;
const DEFAULT_THRESHOLD = Number(process.env.DEFAULT_THRESHOLD ?? 100);

// ===== TELEGRAM POLLING =====
let lastUpdateId = 0;

async function fetchTelegramCommands() {
  ...
  try {
    ...
    for (const update of updates) {
      ...
      if (text.startsWith("/set")) {
        ...
        if (!amount || amount <= 0) {
          await sendTelegram("❌ Invalid input. Example: /set 100");
          continue;
        }

        userThresholdUSDT = amount;

        await sendTelegram(`✅ Threshold set to ${amount} USDT`);
      }

      // ===== RESET =====
      if (text === "/reset") {
        userThresholdUSDT = null;
        await sendTelegram("🔄 Reset to default threshold");
      }
    }
  } catch (err) {
...
// run every 3 sec
setInterval(fetchTelegramCommands, 3000);
...
async function getWalletBalance() {
  try {
    ...
    return Number(usdt?.walletBalance || 0);
  } catch (err) {
...
// ================= TRANSFER =================
async function transferToFunding(amount) {
  try {
    const timestamp = Date.now().toString();
    ...
    const res = await axios.post(
      `${HTTP_BASE_URL}/v5/asset/transfer/inter-transfer`,
      body,
      {
        headers: {
          "X-BAPI-API-KEY": API_KEY,
    ...
    console.log(`💸 Transferred ${amount} USDT to FUNDING`);
    await sendTelegram(`💸 Transferred ${amount} USDT to FUNDING`);
  } catch (err) {
...
// ================= BALANCE MANAGER =================
async function balanceManager() {
  try {
    const balance = await getWalletBalance();
    const threshold = userThresholdUSDT ?? DEFAULT_THRESHOLD;
    ...
    if (balance > threshold) {
      ...
      await transferToFunding(excess);
    }
  } catch (err) {
    console.error("BALANCE MANAGER ERROR:", err.message);
  }
}

// run every 15 sec
setInterval(balanceManager, 15000);
